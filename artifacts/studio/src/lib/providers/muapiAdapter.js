...
// MuAPI provider adapter. Implements the normalized capability interface
// (text→image, image→image, text→video, image→video, video→video, lip-sync,
// file upload, poll). Request shapes, the submit→poll pattern, field mappings,
// and response normalization are preserved exactly from the original client.
//
// Requests are routed through the api-server proxy (/api/v1 -> api.muapi.ai) in
// both dev and production so the browser never hits muapi directly (avoids CORS).
export class MuapiAdapter {
    ...
    getKey() {
        const key = getProviderKey('muapi');
...
export const muapiAdapter = new MuapiAdapter();
